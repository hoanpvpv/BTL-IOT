...
uint8_t ESPCAM_mac[] = {0xd8, 0x13, 0x2a, 0x7c, 0x4c, 0xc4}; // MAC ESP32 nhận
 uint8_t ESP32_mac[] = {0x80, 0xF3, 0xDA, 0x5E, 0xFC, 0x94};//  gốc
//uint8_t ESP32_mac[] = {0x00, 0x70, 0x07, 0x1b, 0xf3, 0x84};// test
...
void setup() {
  ...
  wifiManager.setConfigPortalTimeout(30);  //  10 seconds for config portal if needed nhớ sửa lại
  ...
  Serial.println("Dang lam nong cam bien, xin cho 10s");
  delay(10000);
...
void loop() {
  ...
  if (ledState && !modeCAM && (currentMillis - ledStartTime >= 80000)) {
    digitalWrite(ESPCAM_CONTROL_PIN, LOW);
    ledState = false;
  }
